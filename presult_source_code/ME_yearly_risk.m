function ME_yearly_risk(varargin)
fprintf('\n Example command line: "./presult yearly_risk ME --trained_net <risk_trained_net.mat> --baseline_risk <baseline_risk.txt> --AR <float> --predictive_data <prediction_data.txt> --id_age_data <id_age_data.txt> --beta_one <float> --incidance_rate <inc_rate_blocks.txt>" \n');
p = inputParser;
addParameter(p,'csv','no',@ischar);
addParameter(p,'missing_value','?');
addParameter(p,'id_age','id_age_label.txt',@ischar)
addParameter(p,'trained_net','ME_risk_trained_net.txt',@ischar)
addParameter(p,'baseline_risk','baseline_risk.txt',@ischar)
addParameter(p,'out','yearly_risk_aug16',@ischar);
addParameter(p,'AR','0.6');
addParameter(p,'beta_one','1');
addParameter(p,'risk_years','10');
addParameter(p,'predictive_data','pid_prediction_data.txt',@ischar)
addParameter(p,'incidence_rate','incidence_rate.txt',@ischar)
new_array = varargin(:);
...
load(net,'-mat');
y_test_pred = mixmlpfwd(net,xtest_pred);
save([out,'_y_test_pred_ME.txt'],'y_test_pred','-ascii')
...
y_pred_mat = mixmlpfwd(net,prediction_matrix);
...
r = ri;
retrieved_id_pre = id_age(:,1);
retrieved_id = retrieved_id_pre(locb);
retrieved_id_age = [retrieved_id, retrieved_age];
for i = 2:risk_years
