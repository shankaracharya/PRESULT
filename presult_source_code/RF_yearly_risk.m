function RF_yearly_risk(varargin)
fprintf('\n Example command line: "./presult yearly_risk RF --trained_net <risk_trained_net.mat> --baseline_risk <baseline_risk.txt> --AR <float> --predictive_data <prediction_data.txt> --id_age_data <id_age_data.txt> --beta_one <float> --incidance_rate <inc_rate_blocks.txt>" \n');
p = inputParser;
addParameter(p,'csv','no',@ischar);
addParameter(p,'missing_value','?');
addParameter(p,'id_age','pid_id_age_label.txt',@ischar);
addParameter(p,'trained_net','RF_trained_net.mat');
...
addParameter(p,'predictive_data','prediction_data.txt',@ischar)
addParameter(p,'incidence_rate','incidence_mort_rate.txt',@ischar)
new_array = varargin(:);
...
predictive_test_tmp = load('predict_data_normalized.txt');
predictive_test = predictive_test_tmp(:,1:end);
xtest_pred = predictive_test(:,2:end-1);
load(Random_Forest,'-mat');
y_test_pred = eval_RF(xtest_pred,Random_Forest);
...
y_pred_mat = eval_RF(prediction_matrix,Random_Forest);
...
save([out,'_yearwise_absoulte_risk.txt'], 'p_final', '-ascii');
disp(p_final);
end
